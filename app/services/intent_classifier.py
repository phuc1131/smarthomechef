...
def classify_intent(text: str) -> Tuple[str, float, list]:
    """[PHÂN LOẠI Ý ĐỊNH - PHƯƠNG PHÁP 3: Keyword Fallback Weighted]"""
    if not text:
        return 'unknown', 0.0, []
    ...
    for label, kws_weighted in _INTENT_KEYWORDS.items():
        matches = []
        current_score = 0.0
        max_possible_score = sum(weight for kw, weight in kws_weighted)
        ...
        # Boost score slightly for longer matches
    ...
    confidence = round(min(1.0, float(best_score * 2)), 3) # Scaled for better thresholding
